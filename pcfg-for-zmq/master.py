...
class StateManager:
    ...
    def get_idle_workers(self):
        with self._lock:
            return [{'id': w_id, **w_data} for w_id, w_data in self.workers.items() if w_data.get('identity') and not w_data.get('guessing')]
...
class Master:
    def __init__(self, config):
        ...
        self.pcfg=PCFGGuesser(config=self.config)
        self.queue : PcfgQueue = PcfgQueue(pcfg=self.pcfg)
    ...
    def _run_job(self):
        ...
        while self.state.get_job_status() == "RUNNING":
            ...
            current_idle_workers = self.state.get_idle_workers()
            if not current_idle_workers:
                self.logger.error("No idle workers. Waiting 1 second.")
                time.sleep(1)
                continue
            ...
            node = self.queue.get()
            if node is None:
                self.logger.error("No worker found. Retrying...")
                self.state.stop_job(final_status="FINISHED")
                break

            chunks = self.pcfg.split_structures(node, len(current_active_workers))

            for i, worker in enumerate(current_active_workers):
                if self.state.get_job_status() != "RUNNING":
                    break
                worker_identity = worker['identity']
                structure = chunks[i]
                payload = json.dumps([s.serialize() for s in structure]).encode('utf-8')
