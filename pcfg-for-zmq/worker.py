...
from lib.guess.pcfg.pcfg_guesser import Structure, PCFGGuesser
...
class Worker:
    ...
    def process_candidates(self, candidates_payload):
        ...
        logger.info(f"Received {list(map(str,candidates_payload))} candidates to process.")
        self.guessing = True
        for candidate in self.pcfg.guess(candidates_payload):
            if hashlib.sha256(candidate.encode('utf-8')).hexdigest() == self.target_hash:
                ...
                break
        self.count += 1
        logger.info(f"Processed {self.count} node.")
        self.guessing = False
    ...
    def start(self):
        ...
        try:
            ...
            while True:
                ...
                elif command == 'candidates' and self.running and not self.guessing:
                    self.process_candidates([Structure(**d) for d in json.loads(payload_bytes[0].decode('utf-8'))])
