...
"""
OMEN(마르코프) 모델을 AlphabetGrammar 구조에 맞춰
디스크에 저장하고, OMEN 레벨 계산 함수를 포함합니다.
"""
...
from lib_training.train_output import make_sure_path_exists


def save_omen_rules_to_disk(
    alphabet_grammar,      # AlphabetGrammar 인스턴스
    omen_keyspace,         # Counter(level -> keyspace)
    omen_levels_count,     # Counter(level -> cracked_count)
    num_valid_passwords,
    base_directory,
    program_info
):
