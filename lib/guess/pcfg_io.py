...
def load_indexed_prob_from_db(conn, table_name):
    cursor = conn.cursor()
...
def load_pcfg_data_from_sqlite(db_path):
    from lib.guess.pcfg_guesser import Type
    ...
    try:
        ...
        for table_name, prefix in mappings:
            try:
                data = load_indexed_prob_from_db(conn, table_name)
